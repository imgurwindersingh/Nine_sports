...
const storageRoot = process.env.NETLIFY
  ? path.join("/tmp", "nine-sports-backend")
  : __dirname;

const app = express();
const uploadsDir = path.join(storageRoot, "uploads");
const collectionsFile = path.join(storageRoot, "collections.json");
...
app.use((req, _res, next) => {
  if (req.url === netlifyFunctionPrefix || req.url.startsWith(`${netlifyFunctionPrefix}/`)) {
...
app.get("/", (_req, res) => {
  res.json({
    ...
    note: process.env.NETLIFY
      ? "Netlify is using temporary storage for uploaded files in this demo setup."
